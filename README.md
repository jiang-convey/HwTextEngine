...
本项目把这些需求做成一个统一的命令行工具 `hwtext`，**用自己实现的算法**支撑每一步，
而不是调用现成库函数。

> 说明：本项目的需求来源，是作者此前做的一个课程自动化辅助仓库（编排型工具，大量依赖
> 外部库）。本大作业**重新立项、从零编写算法内核**，与之不共享代码，仅借用真实场景。
...
> **关于"底层实现"**：所有上述算法均不调用任何第三方库；连 `heapq`、`re` 也未使用
> （`heapq` 用手写 `MinHeap` 取代；行/标记识别用手写谓词取代正则）。第三方库
> `pymupdf` / `pdfplumber` **只**出现在 `src/hwtext/io/`，且只负责"把 PDF 解码成字符串"。
...
- **算法来源**：KMP、Aho–Corasick、Levenshtein、Huffman、二叉堆等均为公开的经典算法，
  按教材标准描述实现（参考文献见下），作者理解其原理并能讲解、复现与答辩。
- **作者主导**：选题与问题建模（把"作业文本处理"映射到这组算法）、模块架构（逻辑/界面/
  IO 分层、算法零依赖的边界设计）、流水线设计与示例，由作者构思与决定。

> ⚠️ 提交前请按你**实际**的参与情况据实修改本节（哪些是你独立写的、AI 协助到什么程度），
> 不要原样照抄。如实声明本身符合课程要求；不实声明才违规。
