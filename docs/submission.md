...
| 选题方向 | 算法应用工具（文本处理） |
| 代码仓库 | `https://github.com/<你的GitHub用户名>/HwTextEngine`　（提交前替换为真实链接） |
...
> 需求来源说明：本项目的场景来自作者此前做的一个课程自动化辅助仓库（编排型工具，大量依赖
> 外部库）。本大作业**重新立项、从零编写算法内核**，与之不共享任何代码，仅借用真实场景。
...
- **算法来源**：KMP、Aho–Corasick、Levenshtein、Huffman、二叉堆等均为公开经典算法，按教材
  标准描述实现（参考文献见下），作者理解其原理并能讲解、复现与答辩。
- **作者主导**：选题与问题建模（把"作业文本处理"映射到这组算法）、模块架构（逻辑 / 界面 / IO
  分层、算法零依赖的边界设计）、流水线设计与示例，由作者构思与决定。

> 【提交前请据实修改本节】请按你**实际**的参与情况如实描述（哪些是你独立写的、AI 协助到什么
> 程度）。如实声明本身符合课程要求；不实声明才违规。
